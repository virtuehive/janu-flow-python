...
use janu_flow::ZFError;
pub fn into_py(py: Python, value: janu_flow::Configuration) -> PyObject {
    match value {
        janu_flow::Configuration::Array(arr) => {
        ...
        janu_flow::Configuration::Object(obj) => {
        ...
        janu_flow::Configuration::Bool(b) => b.to_object(py),
        janu_flow::Configuration::Number(n) => n.as_u64().unwrap().to_object(py), //TODO convert to the right number
        janu_flow::Configuration::String(s) => s.to_object(py),
        janu_flow::Configuration::Null => py.None(),
    }
}
...
impl From<&mut janu_flow::Context> for Context {
    fn from(ctx: &mut janu_flow::Context) -> Self {
        Self {
            mode: ctx.mode.clone(),
        }
    }
...
impl TryFrom<janu_flow::DataMessage> for DataMessage {
    type Error = ZFError;
    fn try_from(msg: janu_flow::DataMessage) -> Result<Self, Self::Error> {
        Ok(Self {
            ts: msg.timestamp,
...
impl TryFrom<&mut janu_flow::DataMessage> for DataMessage {
    type Error = ZFError;
    fn try_from(msg: &mut janu_flow::DataMessage) -> Result<Self, Self::Error> {
        Ok(Self {
            ts: msg.timestamp,
            data: msg.data.try_as_bytes()?.to_vec(),
        })
...
impl TryFrom<&mut HashMap<janu_flow::PortId, janu_flow::DataMessage>> for Inputs {
    type Error = ZFError;
    fn try_from(
        rust_inputs: &mut HashMap<janu_flow::PortId, janu_flow::DataMessage>,
...
impl TryFrom<HashMap<janu_flow::PortId, janu_flow::Data>> for Outputs {
    type Error = ZFError;
    fn try_from(
        rust_inputs: HashMap<janu_flow::PortId, janu_flow::Data>,
    ) -> Result<Self, Self::Error> {
...
impl TryInto<HashMap<janu_flow::PortId, janu_flow::Data>> for Outputs {
    type Error = ZFError;
    fn try_into(self: Self) -> Result<HashMap<janu_flow::PortId, janu_flow::Data>, Self::Error> {

        let mut outputs = HashMap::new();
        for (k, v) in self.outputs {
            let data = janu_flow::Data::from_bytes(v);
